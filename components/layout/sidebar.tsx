...
import { usePermissions, useCurrentProfileState } from "@/lib/data/hooks";
            ...
            {!collapsed && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-w-0 overflow-hidden">
                <p className="text-xs font-medium text-foreground truncate">
                  {profile?.full_name ?? (profileLoading ? "Loading…" : "Signed in")}
                </p>
                {/* Never assert a role we do not have. Printing "viewer" for a
                    failed profile read understates what the person can actually do. */}
